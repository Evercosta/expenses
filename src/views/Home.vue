  ...
  computed: {
    totals () {
      ...
      const values = {
        ...
        biggest: {},
        lowest: {},
        qtSpent: exp.length // qt de compras
      }
      if (exp.length) {
        values.totalSpent = exp.map(e => +e.value) // map tira os valores do array
          .reduce((acc, cur) => acc + cur, 0) // reduce somas esses valores, '+' na frente muda para float
  ...
  methods: {
    getData () {
      ...
      ref.on('value', data => {
        ...
        this.expenses = Object.keys(values).map(i => values[i]) // tranformando em array
      })
    }
  }
}
